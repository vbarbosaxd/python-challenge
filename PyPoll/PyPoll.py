...
#Write the text file
Analysis = os.path.join((os.path.abspath(__file__)), '..', 'Analysis', 'PyPoll.txt')
with open(Analysis,"w") as new:
    new.write(f"Election Results")
    new.write("\n")
    new.write(f"Total Votes: {total_votes}")
    new.write("\n")
    new.write(f"Khan: {round(khan_percent,3)}% ({khan_votes})")
    new.write("\n")
    new.write(f"Correy: {round(correy_percent,3)}% ({correy_votes})")
    new.write("\n")
    new.write(f"Li: {round(li_percent, 3)}% ({li_votes})")
    new.write("\n")
    new.write(f"O'Tooley: {round(otooley_percent, 3)}% ({otooley_votes})")
    new.write("\n")
